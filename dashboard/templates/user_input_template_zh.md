...
以下不是需要填写的字段。当输出形式隐含某种写作风格时，生成器从 `writing-guidelines/` 加载匹配的写作指导，填入 `output-template.md` 的 `## Writing Style` 段。如果没有匹配的写作指导，在 Phase 0 头脑风暴阶段根据选题和受众确定写作风格。

可选的写作指导：
- 研究报告 / 技术分析 → 无固定风格（在 Phase 0 确定）

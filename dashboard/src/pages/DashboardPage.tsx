      ...
      {projects.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {projects.map(project => {
            const statusKey = project.hasObservation ? project.status : 'unknown'
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs ${config.bg} ${config.color}`}>
                    ...
                    {project.hasObservation ? t(config.labelKey) : t('dashboard.noObs')}
                  </div>
                </div>

                {project.hasObservation && (
                  <>
                    <div className="mb-3">
